...
namespace Schdular
{
    public partial class Form1 : Form
    {
        ...
        private void draw_nop( int start, int finish)
        {
            ColumnStyle temp = panel.ColumnStyles[panel.ColumnCount - 1];
            panel.ColumnCount++;
            panel.ColumnStyles.Add(new ColumnStyle(temp.SizeType, 30 * (finish - start)));
            panel.Controls.Add(new Label() { Text = "NOP" }, panel.ColumnCount - 1, 0);
            panel.Controls.Add(new Label() { Text = " " + start + "         " + finish }, panel.ColumnCount - 1, 1);
        ...
        private void non_preemptive_priority()
        {
            ...
            while (true)
            {
                ...
                for (int j = 0; j < temp.Length; j++)
                {
                    ...
                    if (temp[j].Arrival_time <= current_time && temp[j].Brust_time != 0)
                    {
                ...
                if (f == current_time)
                {
                    draw_nop(current_time, current_time + 1);
                      current_time++;
        ...
        private void fcfs()
        {
            ...
            foreach (Process p in query)
            {
                ...
                {
                    draw_nop(current_time, p.Arrival_time);
        ...
        private void preemptive_priority()
        {
            ...
            while (true)
            {
                ...
                if (current_process.Brust_time == 0)
                {
                    ...
                    stop = current_time;
                    draw_in_table(current_process, start, stop);
                    // ana 7atet el klam dah wana me4 mot2akedn mno 5als
                    if (x == 0 && max == 99999999 && last_arrived<arrived.Length-1 )
                    {

                        draw_nop(current_time, arrived[last_arrived + 1].Arrival_time);
                        current_time = arrived[last_arrived + 1].Arrival_time;
                        current_process = arrived[last_arrived + 1];
                        start = current_time;
                        f = 1;
                    }
                   
                    else  // ana 7atet el klam dah wana me4 mot2akedn mno 5als
                    {
